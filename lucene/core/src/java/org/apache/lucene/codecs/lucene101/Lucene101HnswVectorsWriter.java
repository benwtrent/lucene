...
import static org.apache.lucene.codecs.lucene101.Lucene101HnswVectorsFormat.DIRECT_MONOTONIC_BLOCK_SHIFT;
import static org.apache.lucene.codecs.lucene101.Lucene101HnswVectorsReader.SIMILARITY_FUNCTIONS;
 ...
 * @lucene.experimental
 */
public final class Lucene101HnswVectorsWriter extends KnnVectorsWriter {

  private static final long SHALLOW_RAM_BYTES_USED =
      RamUsageEstimator.shallowSizeOfInstance(Lucene101HnswVectorsWriter.class);
  ...
  public Lucene101HnswVectorsWriter(
    ...
    String metaFileName =
        IndexFileNames.segmentFileName(
            state.segmentInfo.name, state.segmentSuffix, Lucene101HnswVectorsFormat.META_EXTENSION);

    String indexDataFileName =
        IndexFileNames.segmentFileName(
            state.segmentInfo.name,
            state.segmentSuffix,
            Lucene101HnswVectorsFormat.VECTOR_INDEX_EXTENSION);
    ...
    try {
      ...
      CodecUtil.writeIndexHeader(
          meta,
          Lucene101HnswVectorsFormat.META_CODEC_NAME,
          Lucene101HnswVectorsFormat.VERSION_CURRENT,
          state.segmentInfo.getId(),
          state.segmentSuffix);
      CodecUtil.writeIndexHeader(
          vectorIndex,
          Lucene101HnswVectorsFormat.VECTOR_INDEX_CODEC_NAME,
          Lucene101HnswVectorsFormat.VERSION_CURRENT,
          state.segmentInfo.getId(),
          state.segmentSuffix);
      success = true;
  ...
  private void writeField(FieldWriter<?> fieldData) throws IOException {
    ...
    int[] maxBytesPerLevel = writeGraph(graph);
  ...
  private void writeSortingField(FieldWriter<?> fieldData, Sorter.DocMap sortMap)
    ...
    long vectorIndexOffset = vectorIndex.getFilePointer();
    OnHeapHnswGraph graph = fieldData.getGraph();
    int[] byteOffsetByLevel = new int[graph.numLevels()];
    HnswGraph mockGraph = reconstructAndWriteGraph(graph, ordMap, oldOrdMap, byteOffsetByLevel);
    long vectorIndexLength = vectorIndex.getFilePointer() - vectorIndexOffset;
   ...
   * @param maxBytesPerLevel the byte size of the node connections for each level
   * @return The graph
   * @throws IOException if writing to vectorIndex fails
   */
  private HnswGraph reconstructAndWriteGraph(
      OnHeapHnswGraph graph, int[] newToOldMap, int[] oldToNewMap, int[] maxBytesPerLevel)
      throws IOException {
    ...
    while (nodesOnLevel0.hasNext()) {
      ...
      maxBytes = Math.max(mutateAndCalculateByteSizeWritten(neighbors.size(), neighbors.nodes(), oldToNewMap, maxOrd), maxBytes);
      maxBytesPerLevel[0] = maxBytes;
    }
    for (int level = 1; level < graph.numLevels(); level++) {
      ...
      int maxBytesForLevel = 0;
      for (int node : newNodes) {
        NeighborArray neighbors = graph.getNeighbors(level, newToOldMap[node]);
        maxBytesForLevel = Math.max(mutateAndCalculateByteSizeWritten(neighbors.size(), neighbors.nodes(), oldToNewMap, maxOrd), maxBytesForLevel);
      }
      maxBytesPerLevel[level] = maxBytesForLevel;
    }
      ...
      while (nodes.hasNext()) {
        ...
        if (bytesWritten > maxBytesPerLevel[level]) {
          throw new IllegalStateException(
              "Expected to write " + maxBytesPerLevel[level] + " bytes, but wrote " + bytesWritten);
        }
        // write empty bytes to pad to maxBytes
        for (int i = bytesWritten; i < maxBytesPerLevel[level]; i++) {
          vectorIndex.writeByte((byte) 0);
        }
      }
    }
  ...
  private int mutateAndCalculateByteSizeWritten(int size, int[] nnodes, int[] oldToNewMap, int maxOrd) {
    ...
    return GroupVIntUtil.calculateVintBytes(size) + GroupVIntUtil.calculateNumBytes(nnodes, size);
  }

  private void writeNeighbors(NeighborArray neighbors)
      throws IOException {
  ...
  public void mergeOneField(FieldInfo fieldInfo, MergeState mergeState) throws IOException {
    ...
    try {
      ...
      // we use Lucene101HnswVectorsReader.DenseOffHeapVectorValues for the graph construction
      // doesn't need to know docIds
      // TODO: separate random access vector values from DocIdSetIterator?
      OnHeapHnswGraph graph = null;
      int[] byteSizeByLevel = null;
      if (scorerSupplier.totalVectorCount() > 0) {
        ...
        byteSizeByLevel = writeGraph(graph);
      }
      long vectorIndexLength = vectorIndex.getFilePointer() - vectorIndexOffset;
      writeMeta(
          fieldInfo,
  ...
  private int[] writeGraph(OnHeapHnswGraph graph) throws IOException {
    ...
    int[] maxBytesToWritePerLevel = new int[graph.numLevels()];
    for (int level = 0; level < graph.numLevels(); level++) {
      ...
      for (int node : sortedNodes) {
        ...
        int numBytes = GroupVIntUtil.calculateNumBytes(nnodes, size) + GroupVIntUtil.calculateVintBytes(size);
        sumMaxBytes += numBytes;
        maxBytes = Math.max(maxBytes, numBytes);
      }
      System.out.println("Level " + level + " maxBytes: " + maxBytes + " avgMaxBytes: " + (float)sumMaxBytes / sortedNodes.length);
      maxBytesToWritePerLevel[level] = maxBytes;
      for (int node : sortedNodes) {
        NeighborArray neighbors = graph.getNeighbors(level, node);
        int size = neighbors.size();
        ...
        if (bytesWritten > maxBytes) {
          throw new IllegalStateException(
              "Expected to write " + maxBytes + " bytes, but wrote " + bytesWritten);
        }
        // write empty bytes to pad to maxBytes
        for (int i = bytesWritten; i < maxBytes; i++) {
          vectorIndex.writeByte((byte) 0);
        }
      }
    }
  ...
  private void writeMeta(
    ...
    if (graph == null) {
      ...
      for (int level = 0; level < graph.numLevels(); level++) {
        meta.writeVInt(maxBytesPerLevel[level]);
        NodesIterator nodesOnLevel = graph.getNodesOnLevel(level);
        if (level > 0) {
          ...
          meta.writeGroupVInts(nol, nol.length);
        } else {
