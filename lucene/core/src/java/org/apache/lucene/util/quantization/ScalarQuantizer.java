...
public class ScalarQuantizer {
  ...
  public static final long ramBytesRequiredToCreate(
    ...
    // This is the same no matter the confidence interval
    ...
    // This will gather a sample set of vectors with which to gather not only quantiles are
    // different
    // confidence intervals, but also the true nearest neighbors for each vector
    if (confidenceInterval == 0f) {
      ...
      // The sampled vector values used to gather true nearest neighbors and then later quantize
      ramBytesUsed +=
          RamUsageEstimator.alignObjectSize(
  ...
  public static ScalarQuantizer fromVectorsAutoInterval(
    ...
    int sampleSize = Math.min(totalVectorCount, DYNAMIC_QUANT_SAMPLE_SIZE);
    final float[] quantileGatheringScratch =
        new float[floatVectorValues.dimension() * Math.min(SCRATCH_SIZE, totalVectorCount)];
    int count = 0;
    ...
    if (totalVectorCount <= sampleSize) {
      ...
      int[] vectorsToTake = reservoirSampleIndices(totalVectorCount, DYNAMIC_QUANT_SAMPLE_SIZE);
