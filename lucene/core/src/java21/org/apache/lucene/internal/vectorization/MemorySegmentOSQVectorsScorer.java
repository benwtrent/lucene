...
public final class MemorySegmentOSQVectorsScorer extends OSQVectorsScorer {
    ...
    for (; i < limit; i += FLOAT_SPECIES_128.length()) {
      var ax =
          FloatVector.fromMemorySegment(
              FLOAT_SPECIES_128, memorySegment, offset + i * Float.BYTES, ByteOrder.LITTLE_ENDIAN);
      var lx =
          FloatVector.fromMemorySegment(
                  FLOAT_SPECIES_128,
                  memorySegment,
      ...
      if (similarityFunction == EUCLIDEAN) {
        ...
        res =
            res.add(queryCorrections.additionalCorrection())
                .add(additionalCorrections)
                .sub(centroidDp);
  ...
  private void score256Bulk(
    ...
    for (; i < limit; i += FLOAT_SPECIES_256.length()) {
      var ax =
          FloatVector.fromMemorySegment(
              FLOAT_SPECIES_256, memorySegment, offset + i * Float.BYTES, ByteOrder.LITTLE_ENDIAN);
      var lx =
          FloatVector.fromMemorySegment(
                  FLOAT_SPECIES_256,
                  memorySegment,
