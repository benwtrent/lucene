...
final class PanamaVectorUtilSupport implements VectorUtilSupport {
  ...
  @Override
  public void int4BitDotProductBulk(byte[] q, byte[] d, int size, int count, long[] output) {
    assert q.length == size * 4;
    // 128 / 8 == 16
    if (size >= 16 && PanamaVectorConstants.HAS_FAST_INTEGER_VECTORS) {
      if (VECTOR_BITSIZE >= 256) {
        int4BitDotProductBulk256(q, d, size, count, output);
        return;
      } else if (VECTOR_BITSIZE == 128) {
        int4BitDotProductBulk128(q, d, size, count, output);
        return;
      }
    }
    DefaultVectorUtilSupport.int4BitDotProductBulkImpl(q, d, size, count, output);
  }

  public static void int4BitDotProductBulk128(
  ...
  public static void int4BitDotProductBulk256(
      byte[] q, byte[] d, int size, int count, long[] output) {
    for (int iter = 0; iter < count; iter++) {
      long subRet0 = 0;
      long subRet1 = 0;
